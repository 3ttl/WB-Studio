...
作为一位医学在读博士，跑蛋白和蛋白数据的处理真的耗费了我太多的心力。好在现在有了 ChatGPT 的支持，尤其是 GPT6 的出现，帮助我完成了这次的 vibe coding，完成了这样一个粗糙的小软件，来实现我对所有蛋白条带的管理和分析。这个软件可以轻松一览你所有的条带并分析灰度值，并且为每一条目的条带匹配上对应的内参或者另一条你要的作为分母的条带，快速帮你判断趋势；此外还支持一键导出原始数据的功能。相信大家在发稿前，整理原始数据文件也要花费大量的精力吧，本软件支持一键导出排好版的原始数据文件，轻松解放大家的双手。

因为是 vibe coding 的作品，必然还有很多 bug 和可以优化的地方，我自己也会慢慢用这个软件，有时间的话会逐渐迭代。大家有任何的问题都欢迎在 Issue 里面提出。

请格外注意，本软件免费公开在这里，不是拿给大家去卖的，请大家尊重我的劳动成果，也尊重我一天付出了重置两次 Pro 5x 额度的高昂代价，但是欢迎将这个软件分享给你身边的人，希望能为大家的科研生活带来一些便利。

P.S. 作为自己发布的第一个作品，还是有点小紧张的，望大家轻喷。如果大家喜欢的话，可以请我喝杯咖啡吗？（赞赏二维码在最后。）
...
| **统计分析** | 共用候选图库，读取工作台已计算的比值，按实验组整理独立样本，支持同一张膜上的多个生物样本 | 均值 ± SD、单个数据点、统计检验结果、可复制到 Excel 的表格、可复用的图片组合 |
| **内参调齐** | 同时查看多个内参的条带图和泳道柱状图，按对应样本汇总信号 | 再次上样的建议体积、补加 1× SDS 体积 |
| **收藏总览** | 自动汇总收藏图片的所有选框，按实验和蛋白排列，每行最多三张，组名对齐泳道；支持折叠、排序和多选 | 工作台显示效果的选框截图，可直接进入原始数据 PDF 导出 |
| **原始数据导出** | 按实验 → 蛋白 → 图片组合整理，折叠目录、拖动排序，关联化学发光图、背景图和合成图 | 三列排列的白底 PDF；统计页另可导出保留原文件的 ZIP |

## 界面预览

工作台、筛选与收藏总览截图来自 v0.9.0 的实际界面；灰度表格截图来自 v0.9.1。图片和名称均为合成测试素材，不代表真实实验结果。
...
![收藏总览：按实验与蛋白排列选框截图，支持多选导出](docs/images/favorites-overview.png)

![灰度与比值：按校正信号绘制泳道柱状图](docs/images/signal-bars.png)
...
可先点击 **“自带测试示例”**。软件附带 3 张 P62 目的图和 3 张 ACTB 内参图，每张有 4 个泳道，已预设目的图与内参关系。

1. 依次打开 ACTB 内参图，核对条带选框，点击 **“确认并测量”**。
2. 再对 P62 目的图完成相同操作，查看逐泳道的目的蛋白 / 内参比值。
3. 进入 **“统计分析”**，选择 3 张 P62 图片及其选框。
4. 核对每组 3 条有效比值，选择数据口径和统计方法。条件满足时自动生成比较，也可点击 **“生成比较”**。
5. 点击 **“复制数据到 Excel”**，在 Excel 中粘贴。
6. 填写名称并点击 **“保存组合”**，到 **“原始数据导出”** 预览并导出 PDF。

示例生成来源和发行包检查见 [示例来源与隐私检查](docs/示例来源与隐私检查.md)。
...
点击 **“添加文件夹”**，软件递归扫描子文件夹并显示进度。再次点击 **“重新扫描”** 时，会比较已登记文件的路径、大小和修改时间；只有新增或发生变化的图片才读取并校验内容指纹，未变化的图片直接跳过，只导入新增或发生变化的文件。左侧图库与右侧工作区独立滚动，支持文件名搜索、两级排序、日期折叠和图片信息查看。

**图片类型、文件格式、蛋白名称、实验组别**均支持下拉多选：同一栏任意匹配，不同栏同时满足。统计分析页使用同一图库，并可按收藏状态筛选。

右键图片或日期分栏可删除图库记录。确认框中默认 **仅移除链接**，也可选择 **同时将本地文件移入 Windows 回收站**。日期删除仅处理当前筛选下列出的图片，保留其他文件；同内容的其他路径副本也会保留。通过“恢复链接”可找回已移除记录和标注。

eBlot 文件名分类：M 开头为背景图，Merge 开头为合成图，S 开头为化学发光曝光图；文件名包含 8bit 或 16bit 时按对应位深显示。实际图像仍会检查文件容器和像素位深。
...
| `Chemiluminescence`，16-bit（含 `.raw16` 分析原图） | 化学发光 · 16-bit 图 |
| `Composite` | 合成图 |
...
图片类型使用不重复的位深分类；可同时勾选化学发光或背景的 8-bit 与 16-bit 图。条带预览框固定为 **16:9**，预览区与右侧显示调节区按 **3:2** 分配宽度；直方图、低值、高值和伽马滑块可与图像同时查看，ImageJ 定量参数独立放在下方。
...
| 实际实验布局 | 预设与统计选择 |
|---|---|
| 3 张或更多膜，每张膜每组各 1 个生物样本 | 每组样本数设为 1；在统计页选择所有目的图，自动识别跨图重复 |
...
“灰度与比值”表格在校正信号与分母信号之间显示**灰度柱状图**。同一选框各泳道共用线性刻度，柱长依据背景校正信号；未测量显示“—”，零值无柱长，负值向零线左侧显示。图表随选框和有效测量更新，复制当前结果时仍保留原有数值列。

### 4. 统计与保存组合

统计页自动读取工作台已有的有效比值，自动识别跨图重复或单膜组内重复，按组纳入 **至少 3 条有效记录**。可使用原始比值，或相对于所选对照组均值的比值。四个或更多有效重复会全部纳入。

每张图片会显示当前口径的匹配与纳入数量。若只纳入部分图片，请检查图片下方的提示：实验、目的蛋白、内参蛋白标签和定量参数必须对应。只应修正实际填错的标签，不能将不同内参强行标成相同名称。
...
**收藏总览**会自动显示收藏图片中的所有矩形选框，沿用工作台当前保存的对比度、伽马、反相和旋转效果。按实验 → 蛋白排列，同一蛋白每行最多三张，超过后换行；组名显示在每张条带上方，对齐各泳道位置。可折叠实验或蛋白标题，拖动或点击箭头排序，顺序、折叠和勾选会自动保存。

勾选条带后点击 **“导出原始数据”**，进入与原始数据导出页共用的参数界面，无须先保存统计组合。可选择页面、图片显示方式和关联背景／合成图，再预览 PDF。导出按总览中的实验、蛋白和条带顺序排列；每个实验从新页开始。每次最多 72 个选框、24 个蛋白排版行。未画选框的收藏图会显示提示，补齐选框及实验、蛋白名称后可纳入导出。
...
本版统计支持每组至少 3 条有效记录。自动识别、饱和情况、条带位置和生物学重复来源仍需人工核对。使用 ImageJ 不代表整个软件或某项分析结果获得期刊认证。
...
<img width="2190" height="819" alt="赞赏二维码" src="https://github.com/user-attachments/assets/a500bba5-bc64-46bd-a85f-9bb1219374f9" />
