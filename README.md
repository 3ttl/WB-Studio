...
正式发布后，请在本仓库的 **Releases** 中下载 Windows 便携包，例如 `WB_Studio_v0.8.2_Windows_x64.zip`。GitHub 自动提供的 `Source code (zip)` 是源码快照，不是可直接运行的 Windows 软件包。当前 README 尚未配置公开下载地址。
...
桌面外壳使用 C# / WinForms / WebView2，界面使用 HTML、CSS 和 JavaScript；Python 本地服务负责图像读取、SQLite 存储与 PDF 导出，Java 桥接实际 ImageJ。服务绑定本机回环地址，不依赖云端处理实验图片。
...
├─ shell/          Windows 桌面外壳
├─ web/            界面与交互
├─ imagej/         ImageJ 桥接源码、版本清单与许可说明
├─ demo/           合成示例图片与预设
└─ *.py            本地服务、图像处理与导出模块
docs/              使用指南、版本说明和发布准备清单
tests/             测试源码及本地验证产物
build/             构建脚本及本机生成的运行包、备份等
```

构建步骤见 [构建说明](docs/构建说明.md)，ImageJ 桥接说明见 [src/imagej/THIRD_PARTY_NOTICES.md](src/imagej/THIRD_PARTY_NOTICES.md)。

构建脚本支持指定依赖目录，使用同版本的构建依赖包准备私有 Python、ImageJ / Java 与 WebView2 SDK。修改 Java 桥接时提供 JDK 重新编译；运行回归测试还需要 Node.js。普通使用者请使用完整便携包。实际验证范围见 [发布验证](docs/发布验证.md)。
...
WB Studio 自身的项目许可证尚待作者确定；当前根目录未提供 `LICENSE`。确定许可证后，应在此处标明并随仓库及发布包分发，不能仅凭仓库公开推定使用、修改或再分发许可。

感谢 ImageJ 及其他第三方组件。组件许可独立保留，详见：

* [桌面与 Python 组件说明](build/THIRD_PARTY_NOTICES.md)

* [ImageJ 与 Java 运行时说明](src/imagej/THIRD_PARTY_NOTICES.md)

* [ImageJ 版本与校验清单](src/imagej/manifest.json)
